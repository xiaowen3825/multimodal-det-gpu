...
def build_dataloader(cfg: dict, split: str = "train") -> DataLoader:
    ...
    # GPU优化: 启用pin_memory加速CPU->GPU数据传输
    pin_memory = gpu_cfg.get("pin_memory", True) and torch.cuda.is_available()
    ...
    loader = DataLoader(
        ...
        persistent_workers=True if data_cfg.get("num_workers", 8) > 0 else False,
    )
