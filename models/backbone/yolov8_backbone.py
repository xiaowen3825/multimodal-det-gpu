...
class YOLOv8Backbone(nn.Module):
    ...
    def _load_pretrained(self):
        ...
        自动下载并转换ultralytics格式权重到本项目格式。
        权重映射关系:
            ultralytics model.0  -> self.stem
            ultralytics model.1  -> self.stage1[0] (Conv)
            ultralytics model.2  -> self.stage1[1] (C2f)
            ultralytics model.3  -> self.stage2[0] (Conv)
            ultralytics model.4  -> self.stage2[1] (C2f)
            ultralytics model.5  -> self.stage3[0] (Conv)
            ultralytics model.6  -> self.stage3[1] (C2f)
            ultralytics model.7  -> self.stage4[0] (Conv)
            ultralytics model.8  -> self.stage4[1] (C2f)
            ultralytics model.9  -> self.stage4[2] (SPPF)
        """
        import logging
        ...
        try:
            ...
            src_model = yolo.model.model
        except Exception as e:
            logger.warning(f"Failed to load ultralytics YOLOv8s: {e}")
            return
        ...
        new_state = {}
        for src_key, param in src_state.items():
            parts = src_key.split(".")
            if len(parts) < 2:
                continue
            layer_idx = parts[0]
            if layer_idx in layer_mapping:
                rest = ".".join(parts[1:])
                new_key = f"{layer_mapping[layer_idx]}.{rest}"
        ...
        logger.info(
            f"Loaded {len(loaded_keys)}/{len(model_state)} backbone params "
            f"from ultralytics YOLOv8s (skipped {len(skipped_keys)})"
        )
