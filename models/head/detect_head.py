...
class DFL(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: [B, 4*reg_max, A] (A为anchor数)

        Returns:
            [B, 4, A]
        """
        b, _, a = x.shape  # batch, 4*reg_max, anchors
        # [B, 4*c1, A] -> [B*4, c1, A] -> softmax在c1维度 -> conv降维
        x = x.reshape(b * 4, self.c1, a)    # [B*4, c1, A]
        x = x.unsqueeze(-1)                  # [B*4, c1, A, 1]  (作为Conv2d的输入)
        x = F.softmax(x, dim=1)              # 在c1维度softmax
        x = self.conv(x)                     # [B*4, 1, A, 1]
        x = x.reshape(b, 4, a)              # [B, 4, A]
        return x
...
class DecoupledHead(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.embed_dim = embed_dim  # 统一的分类嵌入维度
        ...
        for i, ch in enumerate(in_channels):
            ...
            # 投影到统一的text embedding空间 (所有尺度统一到embed_dim维度)
    ...
    def forward(
        ...
        if isinstance(self.dfl, DFL):
            bbox_decoded = self.dfl(bbox_preds.transpose(1, 2)).transpose(1, 2)  # [B, total_anchors, 4]
        else:
            bbox_decoded = bbox_preds

        return {
            "cls_scores": cls_scores,
            "bbox_preds": bbox_decoded,
