...
class ChannelAffinityAttention(nn.Module):
    ...
    3. 注意力: 视觉query与文本key计算权重 -> 加权聚合文本value
    4. 生成通道调制权重 -> 调制视觉特征

    复杂度: O(C*N)，远低于标准cross-attention的 O(HW*N*C)
    ...
    def __init__(self, vis_channels: int, text_dim: int, reduction: int = 4):
        ...
        self.text_key_proj = nn.Sequential(
            nn.Linear(text_dim, mid_channels),
            nn.ReLU(inplace=True),
            nn.Linear(mid_channels, vis_channels),
        )

        self.text_val_proj = nn.Sequential(
            nn.Linear(text_dim, mid_channels),
            nn.ReLU(inplace=True),
            nn.Linear(mid_channels, vis_channels),
        )

        self.vis_query_proj = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(1),
            nn.Linear(vis_channels, vis_channels),
        )

        self.channel_mlp = nn.Sequential(
            nn.Linear(vis_channels, vis_channels // 4),
            nn.ReLU(inplace=True),
            nn.Linear(vis_channels // 4, vis_channels),
        )
    ...
    def forward(
        ...
        Returns:
            text_modulated: 文本调制后的视觉特征 [B, C, H, W]
        """
        b, c, h, w = visual_feat.shape

        vis_query = self.vis_query_proj(visual_feat)  # [B, C]
        text_keys = self.text_key_proj(text_feat)     # [B, N, C]
        text_vals = self.text_val_proj(text_feat)     # [B, N, C]

        attn_scores = torch.bmm(
            vis_query.unsqueeze(1),        # [B, 1, C]
            text_keys.transpose(1, 2),     # [B, C, N]
        ) / self.temperature               # [B, 1, N]

        if text_mask is not None:
            mask = text_mask.unsqueeze(1).float()  # [B, 1, N]
            attn_scores = attn_scores.masked_fill(mask == 0, float("-inf"))

        attn_weights = F.softmax(attn_scores, dim=-1)  # [B, 1, N]

        text_aggregated = torch.bmm(attn_weights, text_vals)  # [B, 1, C]
        text_aggregated = text_aggregated.squeeze(1)           # [B, C]

        channel_weights = self.channel_mlp(text_aggregated)    # [B, C]
        channel_weights = torch.sigmoid(channel_weights)
        channel_weights = channel_weights.unsqueeze(-1).unsqueeze(-1)  # [B, C, 1, 1]

        text_modulated = visual_feat * channel_weights
