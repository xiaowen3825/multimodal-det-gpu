...
logger = logging.getLogger("multimodal-det")


def _text_collate_fn(batch):
    """文本数据的collate函数（必须是顶层函数，以支持多进程序列化）。"""
    return batch
...
class DistillTrainer:
    ...
    def train(self, text_file: str, num_samples: int = 100000):
        ...
        dataloader = DataLoader(
            ...
            shuffle=True,
            num_workers=0,  # 使用主进程加载，避免多进程序列化问题
            drop_last=True,
            collate_fn=_text_collate_fn,  # 使用顶层函数，兼容多进程
        )
        ...
        import sys
        print(f"[DEBUG] Training starting, total_steps={total_steps}", flush=True)
        ...
        for epoch in range(self.epochs):
            ...
            for step, texts in enumerate(dataloader):
                if step == 0 and epoch == 0:
                    print(f"[DEBUG] First batch received, {len(texts)} texts", flush=True)
                ...
                if step == 0 and epoch == 0:
                    print(f"[DEBUG] Teacher forward done, shape={teacher_embeds.shape}", flush=True)

                # Student前向
                if step == 0 and epoch == 0:
                    print("[DEBUG] Student forward starting...", flush=True)
                student_embeds = self.student(texts=texts)  # [B, D]
                if step == 0 and epoch == 0:
                    print(f"[DEBUG] Student forward done, shape={student_embeds.shape}", flush=True)

                # 计算损失
                loss, losses = self._compute_loss(student_embeds, teacher_embeds)

                if step == 0 and epoch == 0:
                    print(f"[DEBUG] Loss computed: {loss.item():.4f} (feat={losses['feat_loss']:.4f}, align={losses['align_loss']:.4f})", flush=True)
                ...
                if step == 0 and epoch == 0:
                    print("[DEBUG] Backward done, updating weights...", flush=True)
                ...
                if step == 0 and epoch == 0:
                    print("[DEBUG] Step 1 complete! Training loop is running.", flush=True)
