...
class LightTextEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._tokenizer_cache = None  # 缓存tokenizer，避免重复加载
    ...
    def _tokenize(self, texts: list[str]) -> tuple[torch.Tensor, torch.Tensor]:
        """使用CLIP tokenizer进行tokenize（缓存tokenizer实例）。"""
        if self._tokenizer_cache is None:
            try:
                from transformers import CLIPTokenizer
                from models.text_encoder.clip_text_encoder import CLIPTextEncoder
                local_path = CLIPTextEncoder._resolve_local_path(
                    "openai/clip-vit-base-patch32"
                )
                self._tokenizer_cache = CLIPTokenizer.from_pretrained(
                    local_path, local_files_only=True
                )
            except ImportError:
                raise ImportError("Please install transformers: pip install transformers")
        tokens = self._tokenizer_cache(
            texts,
            padding="max_length",
            truncation=True,
