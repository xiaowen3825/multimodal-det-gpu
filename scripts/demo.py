...
def parse_args():
    parser = argparse.ArgumentParser(description="Multimodal Detection Demo (GPU)")
...
def main():
    ...
    for img_path in image_paths:
        ...
        # 后处理（移回CPU）
        cls_scores = outputs["cls_scores"][0].cpu()   # [A, N]
        bbox_preds = outputs["bbox_preds"][0].cpu()   # [A, 4]
        objectness = outputs["objectness"][0].cpu()   # [A, 1]
        anchor_points = outputs["anchor_points"].cpu()
        stride_tensor = outputs["stride_tensor"].cpu()
        ...
        if mask.sum() > 0:
            ...
            # 解码框
            det_anchors = anchor_points[mask] * stride_tensor[mask]
            det_preds = bbox_preds[mask]
            det_boxes = torch.zeros(mask.sum(), 4)
            det_boxes[:, 0] = det_anchors[:, 0] - det_preds[:, 0] * stride_tensor[mask, 0]
            det_boxes[:, 1] = det_anchors[:, 1] - det_preds[:, 1] * stride_tensor[mask, 0]
            det_boxes[:, 2] = det_anchors[:, 0] + det_preds[:, 2] * stride_tensor[mask, 0]
            det_boxes[:, 3] = det_anchors[:, 1] + det_preds[:, 3] * stride_tensor[mask, 0]
