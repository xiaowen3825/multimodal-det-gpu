...
# 强制HuggingFace离线模式，避免from_pretrained联网检查更新导致卡住
...
def parse_args():
    parser = argparse.ArgumentParser(description="Multimodal Detection Training (GPU)")
...
def train_detection(cfg: dict, resume: str, logger):
    ...
    logger.info("=== Detection Training Mode (GPU) ===")
...
def train_distill(cfg: dict, logger):
    ...
    print("[DEBUG] Building Teacher...", flush=True)
    ...
    print("[DEBUG] Teacher built (lazy load).", flush=True)

    # 构建Student
    print("[DEBUG] Building Student...", flush=True)
    ...
    print("[DEBUG] Student built.", flush=True)
